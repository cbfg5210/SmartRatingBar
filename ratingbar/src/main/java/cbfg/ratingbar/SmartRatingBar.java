...
public class SmartRatingBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (mRatingDrawable == null && mRatingBackgroundDrawable == null) {
            mRatingDrawable = ContextCompat.getDrawable(context, R.drawable.rating_ic_star_full);
            mRatingBackgroundDrawable = ContextCompat.getDrawable(context, R.drawable.rating_ic_star_empty);
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        return mIndicator || super.dispatchTouchEvent(event);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (mOrientation == LinearLayout.HORIZONTAL) {
                ...
                int coverStarNum = (int) (coverHeight / (drawableHeight + mGapSize));
        ...
        if (mOrientation == LinearLayout.HORIZONTAL) {
            ...
            if (mRatingDrawable.getIntrinsicHeight() > measuredHeight) {
                ...
                mRatingDrawable.setBounds(
                        0,
                        0,
                        mRatingDrawable.getIntrinsicWidth(),
                        mRatingDrawable.getIntrinsicHeight());
                mRatingBackgroundDrawable.setBounds(
                        0,
                        0,
                        mRatingDrawable.getIntrinsicWidth(),
                        mRatingDrawable.getIntrinsicHeight());
